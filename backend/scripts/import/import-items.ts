...
const DEFAULT_ITEMS_PATH = process.env.FIVETOOLS_ITEMS_PATH ?? path.join(process.cwd(), "data", "items.json");
...
function collectProperties(item: ItemRecord): string[] {
  ...
  (item.property ?? []).forEach((prop) => properties.add(strip5eTokens(prop)));
...
async function importItems(): Promise<void> {
  ...
  if (!items.length) {
    throw new Error(`No items found in ${DEFAULT_ITEMS_PATH}`);
  }

  const filtered = items.filter((item) => ensureAllowedSource(item.source));
